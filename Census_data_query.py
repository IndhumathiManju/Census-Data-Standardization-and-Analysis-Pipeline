...
#     sns.barplot(x=x, y=y, data=df.sort_values(by=x, ascending=False))
#     plt.title(title)
#     plt.xlabel(xlabel)
#     plt.ylabel(ylabel)
#     st.pyplot(plt)  

# def create_distribution_chart(df, column, title, xlabel, ylabel):
#     plt.figure(figsize=(12, 8))
...
def window_1():
    window('Total population of each District',\
            "SELECT District_Code, state_UT, District, Population FROM test.Census_df")
...
def window_2():
    window('Literate Male and Female in each district', \
            "SELECT District_Code, state_UT, District, Literate_Male, Literate_Female FROM test.Census_df")

# window 3 : Percentage of workers in each district

def window_3():
    window('Percentage of workers in each district',"SELECT District_Code, state_UT, District, ROUND((Workers/Population)*100, 2) \
                            AS Percentage_of_Workers FROM test.Census_df")
...
def window_4():
    window('Households with LPG or PNG as Cooking fuel',
            "SELECT District_Code, state_UT, District, LPG_or_PNG_Households FROM test.Census_df")
...
def window_6():
    window("Households with Internet in each District", 
            "SELECT District_Code, state_UT, District, Households_with_Internet FROM test.Census_df")
...
def window_7():
    window('Educational attainment distribution in each district',
            'SELECT District_Code, state_UT, District, Below_Primary_Education, Primary_Education, Middle_Education, \
                ...
                Illiterate_Education, Total_Education FROM test.Census_df')
...
def window_8():
    window('Households with access to various modes of transports in each district', 
            'SELECT District_Code, state_UT, District, Households_with_Bicycle, Households_with_Car_Jeep_Van, \
                Households_with_Radio_Transistor, Households_with_Scooter_Motorcycle_Moped FROM test.Census_df')
...
def window_19():
    window('Households below the poverty line in each state',
            'SELECT state_UT, SUM(Power_Parity_Less_than_Rs_45000) AS Households_below_poverty_line FROM test.Census_df \
            GROUP BY state_UT')
...
def main():
    ...
    select = st.selectbox("Please select a topic to display :",options = options, index = None, key ="select_window",placeholder = 'Choose an option')
